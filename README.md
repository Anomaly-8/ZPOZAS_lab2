...
Программа использует переполнение для повреждения user_key_payload и увеличения поля datalen, что позволяет прочитать содержимое кучи за пределами выделенного буфера и раскрыть адреса ядра.

После обхода повреждается структура simple_xattr в куче, чтобы создать примитив произвольной памяти в память.

Этот примитив используется для модификации глобальной переменной modprobe_path на путь под контролем атакующего, откуда можно запустить произвольный файл с правами root.

- Использование

get_root должно находиться в текущей папке
